...
# Svix webhook test class, rebranded
...
RSpec.describe Reattract::Webhook do
  it "missing id raises error" do
    testPayload = TestPayload.new
    testPayload.headers.delete("svix-id")

    wh = Reattract::Webhook.new(testPayload.secret)

    expect { wh.verify(testPayload.payload, testPayload.headers) }.to raise_error(Reattract::WebhookVerificationError)
  end
  ...
  it "signature verification works with and without prefix" do
    ...
    wh = Reattract::Webhook.new("whsec_" + testPayload.secret)
    json = wh.verify(testPayload.payload, testPayload.headers)
    expect(json[:test]).to eq(2432232314)
  end
